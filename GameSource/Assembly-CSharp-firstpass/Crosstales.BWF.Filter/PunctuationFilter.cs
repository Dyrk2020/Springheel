	...
	public override List<string> GetAll(string text, params string[] sourceNames)
	{
		...
		else
		{
				...
				select capture)
			{
				...
				{
					Debug.Log("Test string contains an excessive punctuation: '" + item.Value + "'");
				}
	...
	public override string ReplaceAll(string text, bool markOnly = false, string prefix = "", string postfix = "", params string[] sourceNames)
	{
		...
		else
		{
				...
				select capture)
			{
				...
				text2 = text2.Replace(item.Value, markOnly ? (prefix + item.Value + postfix) : (prefix + item.Value.Substring(0, _characterNumber) + postfix));
			}
		}
		return text2;
	}
