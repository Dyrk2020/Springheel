...
internal class DraftHybi10Processor : ProtocolProcessorBase
{
	...
	public override void SendHandshake(WebSocket websocket)
	{
		...
		string value = Convert.ToBase64String(SHA1.Create().ComputeHash(Encoding.ASCII.GetBytes(text + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11")));
		...
		stringBuilder.Append("Sec-WebSocket-Origin: ");
	...
	public override void SendData(WebSocket websocket, IList<ArraySegment<byte>> segments)
	{
		...
		for (int i = 0; i < segments.Count; i++)
		{
			...
			list.Add(new ArraySegment<byte>(EncodeDataFrame(2, arraySegment.Array, 0, arraySegment.Count)));
		}
	...
	{
		int num = ((!string.IsNullOrEmpty(closeReason)) ? Encoding.UTF8.GetMaxByteCount(closeReason.Length) : 0) + 2;
		byte[] array = new byte[num];
		int num2 = statusCode / 256;
		int num3 = statusCode % 256;
		array[0] = (byte)num2;
		array[1] = (byte)num3;
	...
	{
		string text = handshakeInfo.Text;
		...
		if (!ProtocolProcessorBase.ExptectedResponseVerbLines.Contains(verbLine))
		{
