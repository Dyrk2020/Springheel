	...
	public static void tokenize(string message, ref TwitchChatMessage chat_message, ref TwitchServerMessage server_message)
	{
						...
						if (text3.Equals("emotes"))
						{
							...
							for (int i = 0; i < array.Length; i++)
							{
								...
								for (int j = 0; j < array2.Length; j++)
								{
									...
									list.Add(new Emote_Entry
									{
										id = id,
										start_index = Convert.ToInt32(array3[0]),
										end_index = Convert.ToInt32(array3[1])
									});
								}
							}
							list.Sort();
		...
		else
		{
			...
			else
			{
				...
				if (chat_message.type != TwitchChatMessage.TwitchChatMessageType.UserMessage && chat_message.type != TwitchChatMessage.TwitchChatMessageType.Whisper)
				{
					return;
				}
