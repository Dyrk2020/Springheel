...
public class Client : Common
{
	...
	private static void ReceiveThreadFunction(ClientConnectionState state, string ip, int port, int MaxMessageSize, bool NoDelay, int SendTimeout, int ReceiveTimeout, int ReceiveQueueLimit)
	{
		...
		{
			Log.Info("Client Recv: failed to connect to ip=" + ip + " port=" + port + " reason=" + ex);
		...
		{
			Log.Error("Client Recv Exception: " + ex5);
		}
		thread?.Interrupt();
		state.Connecting = false;
		state.client?.Close();
	...
	public bool Send(ArraySegment<byte> message)
	{
		if (Connected)
		{
			...
			error("Client.Send: message too big: " + text + ". Limit: " + maxMessageSize);
			return false;
		}
