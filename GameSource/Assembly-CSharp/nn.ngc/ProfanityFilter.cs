...
public sealed class ProfanityFilter : IDisposable
{
	...
	public enum NormalizationOption
	{
		...
		NormalizationOption_UnicodeNormalization = 1,
		[Obsolete("This member is obsoleted. Use SimilarForm instead")]
		NormalizationOption_SimilarForm = 2
	}
	...
	{
		Initialize(ref _profanityFilter, ref _ngcWorkBuffer, checkDesiredLanguage: true).abortUnlessSuccess();
	}

	public ProfanityFilter(bool checkDesiredLanguage)
	{
		Initialize(ref _profanityFilter, ref _ngcWorkBuffer, checkDesiredLanguage).abortUnlessSuccess();
	}
	...
	private static extern Result Initialize(ref IntPtr profanityFilter, ref IntPtr ngcWorkBuffer, [MarshalAs(UnmanagedType.U1)] bool checkDesiredLanguage);
	...
	[DllImport("NintendoSDKPlugin", CallingConvention = CallingConvention.Cdecl, EntryPoint = "nn_ngc_SetNormalizationOption")]
