...
public class ArraySegmentList<T> : IList<T>, ICollection<T>, IEnumerable<T>, IEnumerable where T : IEquatable<T>
{
	...
	internal IList<ArraySegmentEx<T>> Segments => m_Segments;
	...
	public T this[int index]
	{
		get
		{
			...
			int elementInternalIndex = GetElementInternalIndex(index, out segment);
...
public class ArraySegmentList : ArraySegmentList<byte>
{
	...
	public string Decode(Encoding encoding, int offset, int length)
	{
		...
		for (int i = segmentIndex; i < segments.Count; i++)
		{
			...
			decoder.Convert(arraySegmentEx.Array, byteIndex, num4, array, num2, array.Length - num2, flush, out var bytesUsed, out var charsUsed, out var _);
			num2 += charsUsed;
			num += bytesUsed;
			if (num >= length)
			{
