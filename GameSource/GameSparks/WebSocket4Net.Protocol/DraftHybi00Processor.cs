...
internal class DraftHybi00Processor : ProtocolProcessorBase
{
	...
	static DraftHybi00Processor()
	{
		...
		m_Random = new Random();
		CloseHandshake = new byte[2] { 255, 0 };
	...
	{
		int num = Encoding.UTF8.GetMaxByteCount(message.Length) + 2;
	...
	{
		string text = Encoding.UTF8.GetString(GenerateSecKey());
		string text2 = Encoding.UTF8.GetString(GenerateSecKey());
		byte[] array = GenerateSecKey(8);
		m_ExpectedChallenge = GetResponseSecurityKey(text, text2, array);
		...
		stringBuilder.AppendWithCrCf(text2);
