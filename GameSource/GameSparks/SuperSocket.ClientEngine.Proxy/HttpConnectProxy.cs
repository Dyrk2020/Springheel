...
public class HttpConnectProxy : ProxyConnectorBase
{
	...
	{
		m_LineSeparator = Encoding.UTF8.GetBytes("\r\n\r\n");
	}
	...
	{
		GameSparksUtil.Log("Connect1");
		if (remoteEndPoint == null)
		{
			throw new ArgumentNullException("remoteEndPoint");
		}
		if (!(remoteEndPoint is IPEndPoint) && !(remoteEndPoint is DnsEndPoint2))
		{
			throw new ArgumentException("remoteEndPoint must be IPEndPoint or DnsEndPoint", "remoteEndPoint");
		}
		try
		{
			base.ProxyEndPoint.ConnectAsync(ProcessConnect, remoteEndPoint);
		}
	...
	protected override void ProcessConnect(Socket socket, object targetEndPoint, SocketAsyncEventArgs e)
	{
		...
		string text;
		if (targetEndPoint is DnsEndPoint2)
		{
			DnsEndPoint2 dnsEndPoint = (DnsEndPoint2)targetEndPoint;
			text = string.Format("CONNECT {0}:{1} HTTP/1.1\r\nHost: {0}:{1}\r\nProxy-Connection: Keep-Alive\r\n\r\n", dnsEndPoint.Host, dnsEndPoint.Port);
		}
		else
		{
	...
	protected override void ProcessReceive(SocketAsyncEventArgs e)
	{
		...
		StringReader stringReader = new StringReader(Encoding.UTF8.GetString(e.Buffer, 0, num3));
		...
		{
			OnCompleted(new ProxyEventArgs(connectContext.Socket));
		}
	}
}
